...
stable_moving_avg_history = []
global_avg_of_stable_data = []
plt.ion()
...
def update_plot():
    """ MODIFICADO: A função agora apenas exibe os dados pré-calculados. """
    if not time_data: return

    # Atualiza Gráfico de Velocidade (Médias Móveis)
    ...
    if len(speed_data_raw) >= MOVING_AVERAGE_WINDOW_LONG:
        ...
        # A Média Global agora usa sua própria lista de dados, que está em sincronia
        line_global_avg.set_data(time_for_avg_long, global_avg_of_stable_data)
        
        # Atualiza Gráficos de FFT
...
def process_video_feed_with_raft():
    ...
    while plt.fignum_exists(fig.number):
        ...
        if not is_outlier:
            ...
            # --- MODIFICADO: Lógica da Média Global movida para o loop principal ---
            # Garante que a lista da média global só cresça quando um dado válido é adicionado.
            if len(speed_data_raw) >= MOVING_AVERAGE_WINDOW_LONG:
                # Pega o valor mais recente da média móvel longa
                latest_stable_avg = np.mean(speed_data_raw[-MOVING_AVERAGE_WINDOW_LONG:])
                stable_moving_avg_history.append(latest_stable_avg)
                
                # Calcula a média cumulativa desse histórico
                global_avg_of_stable = np.mean(stable_moving_avg_history)
                global_avg_of_stable_data.append(global_avg_of_stable)
