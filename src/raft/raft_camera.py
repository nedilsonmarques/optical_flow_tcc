...
# --- Estruturas de Dados e Setup de Gráficos ---
# (Esta seção inteira permanece inalterada)
time_data = []
speed_data_raw = []
position_data = []
global_avg_speed_data = []
plt.ion()
...
line_global_avg, = ax_speed.plot([], [], '--', color='cyan', linewidth=2, label='Média Global (Total)')
ax_speed.legend()
...
def update_plot():
    ...
    if len(speed_data_raw) >= MOVING_AVERAGE_WINDOW_SHORT:
        moving_avg_short = np.convolve(speed_data_raw, np.ones(MOVING_AVERAGE_WINDOW_SHORT), 'valid') / MOVING_AVERAGE_WINDOW_SHORT
        time_for_avg_short = time_data[MOVING_AVERAGE_WINDOW_SHORT-1:]
        line_smooth_short.set_data(time_for_avg_short, moving_avg_short)
    if len(speed_data_raw) >= MOVING_AVERAGE_WINDOW_LONG:
        moving_avg_long = np.convolve(speed_data_raw, np.ones(MOVING_AVERAGE_WINDOW_LONG), 'valid') / MOVING_AVERAGE_WINDOW_LONG
        time_for_avg_long = time_data[MOVING_AVERAGE_WINDOW_LONG-1:]
        line_smooth_long.set_data(time_for_avg_long, moving_avg_long)
        if len(moving_avg_long) > 1:
            signal = moving_avg_long; N = len(signal)
            T = (time_for_avg_long[-1] - time_for_avg_long[0]) / (N - 1) if N > 1 else 1.0
            yf = np.abs(np.fft.rfft(signal)) / N * 2; yf[0] /= 2
            xf = np.fft.rfftfreq(N, T)
            line_fft_auto.set_data(xf, yf)
            line_fft_fixed.set_data(xf, yf)
    line_global_avg.set_data(time_data, global_avg_speed_data)
...
def process_video_feed_with_raft():
    # (Função principal inalterada, exceto pelo bloco de filtro)
    ...
    current_position_px = 0.0

    prev_frame_np = get_image_from_camera()
    if prev_frame_np is None: return
    ...
    while plt.fignum_exists(fig.number):
        ...
        # --- MODIFICADO: Lógica do Filtro de Outliers agora é Bilateral ---
        is_outlier = False
        if len(speed_data_raw) > MOVING_AVERAGE_WINDOW_SHORT:
            global_mean = np.mean(speed_data_raw)
            global_std = np.std(speed_data_raw)
            
            # Define os limiares superior e inferior
            upper_threshold = global_mean + OUTLIER_STD_DEV_THRESHOLD * global_std
            # Garante que o limiar inferior não seja negativo, pois a velocidade é sempre >= 0
        ...
        if not is_outlier:
            # Se o dado for válido, adiciona ao histórico e atualiza a posição
            elapsed_time = (current_msec - start_msec) / 1000.0
            time_data.append(elapsed_time)
            speed_data_raw.append(current_speed_px_per_s)
            
            current_position_px += current_speed_px_per_s * delta_time_s
            position_data.append(current_position_px)
            
            global_avg_speed = current_position_px / elapsed_time if elapsed_time > 0 else 0
            global_avg_speed_data.append(global_avg_speed)
        ...
        cv2.putText(visualization, f"Position: {current_position_px:.1f} px", (10, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
